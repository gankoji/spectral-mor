...
with safe_open(path, framework='pt', device='cpu') as f:
    # find down_proj layer 0
    for k in f.keys():
        if 'down_proj' in k and 'layers.0.' in k and 'experts' not in k:
...
print(f"  Effective rank: {np.exp(spectral_entropy_of_matrix(W)):.1f}")
print(f"  Full rank min(m,n): {min(W.shape)}")
print(f"  Effective rank ratio: {np.exp(spectral_entropy_of_matrix(W))/min(W.shape):.3f}")
